...
[3.4 **Evolução do Ecossistema (Dezembro 2025 → Março 2026)**](#34-evolução-do-ecossistema-dezembro-2025--março-2026)
...
- `scraper/config/site_urls.yaml` - URLs de raspagem
...
### **3.3.1 Pipeline Event-Driven - Visão Geral**
...
gantt
    ...
    section Scraping
    HTTP POST Airflow → API    :0, 1s
    Scraping + Parse HTML       :1s, 2s
    INSERT PostgreSQL           :2s, 1s
    Publish dgb.news.scraped    :3s, 1s

    section Enrichment
    Push to Enrichment Worker   :4s, 1s
    AWS Bedrock Classification  :5s, 4s
    UPDATE PostgreSQL           :9s, 1s
    Publish dgb.news.enriched   :10s, 1s

    section Embeddings
    Push to Embeddings API      :11s, 1s
    Generate Embedding (768-dim):12s, 2s
    UPDATE PostgreSQL           :14s, 1s
    Publish dgb.news.embedded   :15s, 1s

    section Indexação
    Push to Typesense Worker    :11s, 1s
    Fetch Full Document (PG)    :12s, 1s
    Upsert Typesense            :13s, 1s
```

**Latência total**: **13-15 segundos** (scraping → disponível no portal)

### **3.3.4 Idempotência e Tratamento de Erros**
...
### **3.3.5 Monitoramento e Métricas**
...
### **3.3.6 Comparativo: Batch vs Event-Driven**
...
## **3.4 Evolução do Ecossistema (Dezembro 2025 → Março 2026)**
...
### **3.4.2 Comparativo Arquitetural Detalhado**
...
### **3.4.3 Motivações e Decisões Arquiteturais**
...
### **3.4.4 Impacto Operacional e Métricas**
...
### **3.4.7 Comparativo Visual: Fluxos de Dados**
