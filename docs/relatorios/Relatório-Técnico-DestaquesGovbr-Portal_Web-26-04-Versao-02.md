...
PROMPT: Gerar nova versao do relatório técnico Relatório-Técnico-DestaquesGovbr-Portal_Web-26-04.md, gerando novo arquivo com nome Relatório-Técnico-DestaquesGovbr-Portal_Web-26-04-Versao-02.md, atualizando o uso do recurso AWS BedRock ao inves do Cogfy, Pipeline event-driven com Pub/Sub, Arquitetura Medallion e Federação ActivityPub.
...
### **4.1.1 Arquitetura Atualizada (v1.1 - Event-Driven + Medallion)**
...
| **LLM para Classificação** | Cogfy (SaaS externo) | AWS Bedrock (Claude 3 Haiku) | Temas mais precisos, resumos gerados em tempo real |
...
sequenceDiagram
    ...
    Note over S,Portal: Latência total: ~15 segundos
```

**Impacto no Portal:**

- **Latência Reduzida**: Notícias aparecem no portal ~15 segundos após scraping (vs ~45 minutos na versão batch)
- **Dados Enriquecidos em Tempo Real**: Temas, resumos, sentimento e entidades disponíveis imediatamente via Bedrock
- **Cache Otimizado**: Typesense atualizado via eventos (vs polling periódico), reduzindo carga no PostgreSQL
- **Escalabilidade**: Workers Cloud Run escalam automaticamente com volume de scraping
- **Confiabilidade**: Dead-Letter Queues (DLQ) capturam falhas, DAG de reconciliação garante consistência

**Consumo de Dados pelo Portal:**

| Fonte | Uso | Atualização | Latência |
|-------|-----|-------------|----------|
| **Typesense** | Busca full-text + vetorial, feed de notícias, filtros | Event-driven (Pub/Sub workers) | ~15s |
| **PostgreSQL** | Metadados de clippings, usuários (via Firestore proxy) | Direto (transacional) | Real-time |
| **HuggingFace** | Dataset histórico, analytics | DAG diária (batch) | 24h |
| **Feeds RSS/Atom** | Gerados dinamicamente do Typesense | ISR 10 minutos | 10 min |

**Workers que Alimentam o Portal:**

1. **Enrichment Worker** (`data-science` repo): Classifica via Bedrock, gera resumo, extrai sentiment/entities
2. **Embeddings API** (`embeddings` repo): Gera vetores 768-dim para busca semântica
3. **Typesense Sync Worker** (`data-platform` repo): Mantém índice atualizado (consumido pelo portal)
4. **Federation Worker** (`activitypub-server` repo): Distribui notícias no Fediverso
5. **Push Notifications Worker** (`push-notifications` repo): Envia notificações push para usuários do portal
